# A sample Gemfile
source "https://rubygems.org"

gem 'awestruct', '~> 0.5.0'
gem 'rdiscount', '~> 2.0.7.1', :platform => :ruby
gem 'kramdown', '~> 0.14.2', :platform => :jruby
gem 'asciidoctor', '~> 0.1.1'
gem 'haml', '~> 4.0.1'
# the above are all that should be required, I don't know why we need the others
gem 'coffee-script' # needed until we remove the coffee-script transformer 
gem 'therubyracer', '0.10.0', :platform => :ruby # needed until we remove the coffee-script transformer
gem 'therubyrhino', :platform => :jruby # needed until we remove the coffee-script transformer
gem 'htmlcompressor', '~> 0.0.6'
gem 'uglifier', '~> 2.0.1'
